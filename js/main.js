...
(function ($) {
  ...
  function pageBackground() {
    ...
    if (body.hasClass("image-background")) {
    ...
    } else if (body.hasClass("slideshow-background")) {
      ...
      body.hasClass("video-youtube-background") ||
      body.hasClass("video-youtube-list")
    ) {
      // Video background
      if (!$("html").hasClass("touch") && !body.hasClass("mobile")) {
        // Detect mobile devices
        // Init YOUTUBE PLAYER
        var videoPlayer = $(".video-player");
        if (videoPlayer.length) {
          if (body.hasClass("video-youtube-background")) {
            // Youtube SINGLE video
            videoPlayer.each(function () {
              $(this).mb_YTPlayer();
            });
          } else if (body.hasClass("video-youtube-list")) {
            // Youtube LIST video
            var videos = [
              {
  ...
  if (clock.length) {
    if (clock.hasClass("js-timer-elapsed")) {
      ...
      var dateCurrentYear =
        now.getFullYear() + "/" + month + "/" + day + " " + time;
      var outputDays = moment(dateCurrentYear, dateFormat).diff(
        momentNow,
        "days"
      );
      var outputDaysLabel = outputDays > 5 ? "Days" : "Day";
      ...
      clock
        ...
        .on("update.countdown", function (event) {
          var output = [
            ...
            '<div class="counter-box first"><div class="number">' +
      ...
      // Default countdown
      var date = moment().add("31", "days").format("YYYY/MM/DD");
      clock
        .countdown(date + " 00:00:00")
        .on("update.countdown", function (event) {
          var output = [
